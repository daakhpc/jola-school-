...
import { resultsData, resultsContent } from '../constants';
...
const ResultCard: React.FC<ResultCardProps> = ({ grade, data }) => {
  ...
  const labels = resultsContent.labels;
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 flex-grow flex flex-col border-t-8 border-orange-500">
        ...
        <div className="grid grid-cols-2 gap-4 text-base mb-4 text-gray-700">
            <p><strong>{labels.totalStudents[language]}:</strong> {data.total}</p>
            <p><strong>{labels.passedStudents[language]}:</strong> {data.passed}</p>
            {data.failed !== undefined && <p><strong>{labels.failedStudents[language]}:</strong> {data.failed}</p>}
            {data.compartment !== undefined && <p><strong>{labels.compartment[language]}:</strong> {data.compartment}</p>}
        </div>
        <div className={`text-lg font-bold p-2 text-center rounded-md mb-4 ${isSuccess ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
            {labels.passPercentage[language]}: {data.passPercentage}
        </div>
        <div>
            <h4 className="font-bold text-lg mb-2 text-center text-gray-800">{labels.top3[language]}</h4>
            ...
            ) : <p className="text-center text-gray-500">{labels.noData[language]}</p>}
        </div>
    </div>
  );
};
...
const Results: React.FC = () => {
  ...
  return (
    <SectionWrapper id="results" title={resultsContent.title[language]} className="bg-white">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="w-full md:w-1/2">
            <ResultCard grade={resultsContent.class10[language]} data={currentResult.class10} />
          </div>
          <div className="w-full md:w-1/2">
            <ResultCard grade={resultsContent.class12[language]} data={currentResult.class12} />
          </div>
        </div>
      </div>
    </SectionWrapper>
